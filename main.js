...
global.ytch = "𝓑𝓵𝓪𝓬𝓴 𝓴𝓲𝓷𝓰 𝓵𝓮𝓸𝓷𝓲𝓭𝓪𝓼 (𝓣𝓱𝓮 𝓷𝓮𝓶𝓮𝓼𝓲𝓼 𝓹𝓻𝓲𝓶𝓮)";
...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        if (message.message?.buttonsResponseMessage) {
            ...
            if (buttonId === 'channel') {
                await sock.sendMessage(chatId, {
                    text: '📢 *Join our Channel:*\nhttps://chat.whatsapp.com/DpADtS77s3LKToWpjDQnz1'
                }, { quoted: message });
                return;
        ...
        const adminCommands = ['.mute', '.unmute', '.ban', '.unban', '.promote', '.demote', '.kick', '.tagall', '.tagnotadmin', '.hidetag', '.antilink', '.antitag', '.setgdesc', '.setgname', '.setgpp','.purge'];
        const isAdminCommand = adminCommands.some(cmd => userMessage.startsWith(cmd));

        // List of owner commands
        const ownerCommands = ['.mode', '.autostatus', '.antidelete', '.cleartmp', '.setpp', '.clearsession', '.areact', '.autoreact', '.autotyping', '.autoread', '.pmblocker', '.purge'];
...
case userMessage.startsWith('.purge'):
    // ⚔️ Réaction emoji Queen AI
    try {
        await sock.sendMessage(chatId, { react: { text: '⚔️', key: message.key } });
    } catch (err) {
        console.error('Erreur réaction Purge:', err);
    }

    // 👑 Exécuter la commande purge autonome
    ...
    break;
            ...
            case userMessage.startsWith('.unban'):
                ...
                break;
            // MENU PRINCIPAL (toutes commandes qui montrent le menu complet)
// MENU PRINCIPAL
...
case userMessage === '.general':
case userMessage === '.group':
case userMessage === '.tools':
case userMessage === '.misc':
case userMessage === '.games':
case userMessage === '.social':
case userMessage === '.owner':
case userMessage === '.devinfo':
case userMessage === '.stickmenu':
case userMessage === '.ai': {

    const { menuSystem } = require('./commands/help');
    ...
    await menuSystem(
        ...
        userMessage.replace('.', ''), // 'general', 'group', 'owner', 'ai'

        isGroup,                       // true si message de groupe

        senderIsOwnerOrSudo            // true si propriétaire/sudo
